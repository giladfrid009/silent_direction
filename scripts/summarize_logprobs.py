...
def _read_result_file(path: pathlib.Path) -> pd.DataFrame | None:
    ...
    try:
        ...
        sample_results = data.get("samples")
        if not isinstance(sample_results, dict):
            return None

        # results is a dict of dicts:
        #   key   = benchmark name
        #   value = list of dicts, each dict is for a single sample
        for bench_name, bench_results in sample_results.items():
            if not isinstance(bench_results, list):
                continue

            for i, sample_dict in enumerate(bench_results):
                if "extra_fields" not in sample_dict:
                    return None                
            
                logprobs = [float(l[0]) for l in sample_dict["filtered_resps"]]
...
def collect_from_benchmarks_dir(bench_dir: pathlib.Path, root: pathlib.Path) -> pd.DataFrame | None:
    """
...
def collect_all(root: pathlib.Path) -> pd.DataFrame:
    ...
    out = out.sort_values(["path", "file", "benchmark"], kind="stable").reset_index(drop=True)
...
def main(args) -> int:
    ...
    if len(df) > 0:
        ...
        logger.info(
            "Unique benchmarks=%d,  benchmark dirs=%d",
