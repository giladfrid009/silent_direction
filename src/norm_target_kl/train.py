...
def score_targeted(kl_div: float, target_kl: float, proj_norm: float, tol_factor: float = 2.0) -> float:
    return proj_norm if kl_div <= target_kl * tol_factor else 0.0


def train_norm_targeted_kl(
    ...
    assert loss_kind in {"mse", "mae"}, f"Unsupported loss kind: {loss_kind}"
